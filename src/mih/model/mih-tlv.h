...
#define TLV_TYPE_HELPER_IMPLEM(className,tlvTypeValue)			\
  ...
  {                                                                     \
    static uint8_t typ = ns3::mih::TlvMih::RegisterTlvType ();             \
    return typ;                                                         \
  }                                                                     \
  uint8_t                                                               \
...
namespace ns3 {
  ...
  namespace mih {
    ...
    enum TlvTypeValue_E {
      ...
      TLV_LINK_PARAMETER_QOS = TLV_EXPERIMENTAL + 51,
    };
    ...
    template<typename T>
    uint32_t 
    TlvMih::GetSerializedSize (const std::vector<Ptr<T> > &tList)
    {
    ...
    template <typename T>
    uint32_t
    TlvMih::Deserialize (Buffer &buffer, 
                      std::vector<Ptr<T> > &tList, 
                      uint8_t tlvTypeValue) 
    {
      ...
      totalBytesRead += TlvMih::ReadPayloadLengthField (i, lengthOfLengthField, payloadLengthValue);
