  ...
  namespace mih {
    ...
    LinkIdentifier::GetTlvSerializedSize (void) const
    {
      return m_type.GetTlvSerializedSize () + 
        ...
        TlvMih::GetSerializedSize (m_poaAddress) + 2 // Type + Length
        ;
    }
    void 
    ...
    LinkIdentifier::TlvDeserialize (Buffer &buffer)
    {
      ...
      size  += TlvMih::Deserialize (buffer, m_mobileNodeAddress, TLV_MN_LINK_IDENTIFIER);
      size  += TlvMih::Deserialize (buffer, m_poaAddress, TLV_POA_LINK_IDENTIFIER);
    ...
    {
      return TlvMih::GetSerializedSize<LinkIdentifier> (linkIdentifierList);
    }
    void 
    TlvSerialize (Buffer &buffer, LinkIdentifierList &linkIdentifierList)
    {
      return TlvMih::Serialize<LinkIdentifier> (buffer, linkIdentifierList, TLV_LINK_IDENTIFIER_LIST);
    }
    uint32_t 
    TlvDeserialize (Buffer &buffer, LinkIdentifierList &linkIdentifierList)
    {
      return TlvMih::Deserialize<LinkIdentifier> (buffer, linkIdentifierList, TLV_LINK_IDENTIFIER_LIST);
    }
  } // namespace mih
} // namespace ns3
