...
namespace ns3 {
  namespace mih {
    ...
    Threshold::GetTlvSerializedSize (void) const 
    {
      ...
      payloadLength = TlvMih::GetSerializedSizeU16 () + // m_value;
        TlvMih::GetSerializedSizeU8 (); // m_xdir
    ...
    Threshold::TlvSerialize (Buffer &buffer) const
    {
      ...
      TlvMih::SerializeU16 (buffer, m_value, TLV_THRESHOLD_VALUE);
      TlvMih::SerializeU8 (buffer, static_cast<uint8_t> (m_crossDirection), TLV_THRESHOLD_X_DIRECTION);
    }
    uint32_t 
    Threshold::TlvDeserialize (Buffer &buffer)
    {
      ...
      totalBytesRead += TlvMih::DeserializeU16 (buffer, m_value, TLV_THRESHOLD_VALUE);
      uint8_t xdir = 0;
      totalBytesRead += TlvMih::DeserializeU8 (buffer, xdir, TLV_THRESHOLD_X_DIRECTION);
    ...
    {
      return TlvMih::GetSerializedSize<Threshold> (thresholdList);
    }
    void
    TlvSerialize (Buffer &buffer, const ThresholdList &thresholdList)
    {
      TlvMih::Serialize<Threshold> (buffer, thresholdList, TLV_THRESHOLD_LIST);
    }
    uint32_t
    TlvDeserialize (Buffer &buffer, ThresholdList &thresholdList)
    {
      return TlvMih::Deserialize<Threshold> (buffer, thresholdList, TLV_THRESHOLD_LIST);      
    }
  } // namespace mih
} // namespace ns3
