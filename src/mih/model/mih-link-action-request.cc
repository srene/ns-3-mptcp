  ...
  namespace mih {
    ...
    LinkActionRequest::GetTlvSerializedSize (void) const
    {
      ...
      payloadLength = m_linkIdentifier.GetTlvSerializedSize () +
        TlvMih::GetSerializedSize (m_poaAddress) +
    ...
    LinkActionRequest::TlvSerialize (Buffer &buffer) const
    {
      ...
      TlvMih::Serialize (buffer, m_poaAddress, TLV_POA_LINK_IDENTIFIER);
      m_linkAction.TlvSerialize (buffer);
      TlvMih::SerializeU64 (buffer, m_linkActionExecutionDelay, TLV_TIME_INTERVAL);
    }
    uint32_t 
    LinkActionRequest::TlvDeserialize (Buffer &buffer)
    {
      ...
      TlvMih::Deserialize (buffer, m_poaAddress, TLV_POA_LINK_IDENTIFIER);
      totalBytesRead += m_linkAction.TlvDeserialize (buffer);
      totalBytesRead += TlvMih::DeserializeU64 (buffer, m_linkActionExecutionDelay, TLV_TIME_INTERVAL);
    ...
    {
      return TlvMih::GetSerializedSize<LinkActionRequest> (linkActionRequestList);
    }
    void
    TlvSerialize (Buffer &buffer, const LinkActionRequestList &linkActionRequestList)
    {
      TlvMih::Serialize<LinkActionRequest> (buffer, linkActionRequestList, TLV_LINK_ACTION_REQUEST_LIST);
    }
    uint32_t
    TlvDeserialize (Buffer &buffer, LinkActionRequestList &linkActionRequestList)
    {
      return TlvMih::Deserialize<LinkActionRequest> (buffer, linkActionRequestList, TLV_LINK_ACTION_REQUEST_LIST);    
    }
  } // namespace mih
} // namespace ns3
