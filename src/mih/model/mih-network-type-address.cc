  ...
  namespace mih {
    ...
    NetworkTypeAddress::GetTlvSerializedSize (void) const
    {
      return m_linkType.GetTlvSerializedSize ()  + 
        TlvMih::GetSerializedSize (m_linkAddress) + 
        1 + 1 // Type + Length
        ;
    }
    void 
    ...
    NetworkTypeAddress::TlvSerialize (Buffer &buffer) const
    {
      ...
      TlvMih::Serialize (buffer, m_linkAddress, TLV_LINK_ADDRESS);
    }
    uint32_t 
    NetworkTypeAddress::TlvDeserialize (Buffer &buffer)
    {
      ...
      size  += TlvMih::Deserialize (buffer, m_linkAddress, TLV_LINK_ADDRESS);
    ...
    {
      return TlvMih::GetSerializedSize<NetworkTypeAddress> (networkTypeAddressList);
    }
    void 
    TlvSerialize (Buffer &buffer, NetworkTypeAddressList &networkTypeAddressList)
    {
      return TlvMih::Serialize<NetworkTypeAddress> (buffer, networkTypeAddressList, TLV_NETWORK_TYPE_ADDRESS_LIST);
    }
    uint32_t 
    TlvDeserialize (Buffer &buffer, NetworkTypeAddressList &networkTypeAddressList)
    {
      return TlvMih::Deserialize<NetworkTypeAddress> (buffer, networkTypeAddressList, TLV_NETWORK_TYPE_ADDRESS_LIST);
    }
  } // namespace mih
} // namespace ns3
