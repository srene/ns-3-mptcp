...
namespace ns3 {
  namespace mih {
    ...
    LinkAction::GetTlvSerializedSize (void) const
    {
      ...
      payloadLength = TlvMih::GetSerializedSizeU8 () + 
    ...
    LinkAction::TlvSerialize (Buffer &buffer) const
    {
      ...
      TlvMih::SerializeU8 (buffer, static_cast<uint8_t> (m_type), TLV_LINK_ACTION);
      TlvMih::SerializeU8 (buffer, m_actionAttribute, TLV_LINK_ACTION_ATTRIBUTE);
    }
    uint32_t 
    LinkAction::TlvDeserialize (Buffer &buffer)
    {
      ...
      uint8_t typ = 0;
      totalBytesRead += TlvMih::DeserializeU8 (buffer, typ, TLV_LINK_ACTION);
      m_type = static_cast<enum LinkAction::Type> (typ);
      totalBytesRead += TlvMih::DeserializeU8 (buffer, m_actionAttribute, TLV_LINK_ACTION_ATTRIBUTE);
