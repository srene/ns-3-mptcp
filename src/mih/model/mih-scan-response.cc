...
namespace ns3 {
  namespace mih {
    ...
    ScanResponse::GetTlvSerializedSize (void) const
    {
      ...
      payloadLength = TlvMih::GetSerializedSize (m_poaLinkAddress) +
    ...
    ScanResponse::TlvSerialize (Buffer &buffer) const
    {
      ...
      TlvMih::Serialize (buffer, m_poaLinkAddress, TLV_POA_LINK_IDENTIFIER);
    ...
    ScanResponse::TlvDeserialize (Buffer &buffer)
    {
      ...
      totalBytesRead += TlvMih::Deserialize (buffer, m_poaLinkAddress, TLV_POA_LINK_IDENTIFIER);
    ...
    {
      return TlvMih::GetSerializedSize<ScanResponse> (scanResponseList);
    }
    void
    TlvSerialize (Buffer &buffer, const ScanResponseList &scanResponseList)
    {
      TlvMih::Serialize<ScanResponse> (buffer, scanResponseList, TLV_SCAN_RESPONSE_LIST);
    }
    uint32_t
    TlvDeserialize (Buffer &buffer, ScanResponseList &scanResponseList)
    {
      return TlvMih::Deserialize<ScanResponse> (buffer, scanResponseList, TLV_SCAN_RESPONSE_LIST);    
    }
  } // namespace mih
} // namespace ns3
