...
namespace ns3 {
  namespace mih {
				   ...
				   MobilityManagementSupport mbbSupport)
    {
      ...
      TlvMih::Serialize (buffer, oldAR, TLV_OLD_ACCESS_ROUTER);
      TlvMih::Serialize (buffer, newAR, TLV_NEW_ACCESS_ROUTER);
      TlvMih::SerializeBool (buffer, ipRenewalFlag, TLV_IP_RENEWAL_FLAG);
                                          ...
                                          LinkGoingDownReason reasonCode)
    {
      ...
      m_session->GetToMihfId ().TlvSerialize (buffer);
      linkIdentifier.TlvSerialize (buffer);
      TlvMih::SerializeU64 (buffer, timeInterval, TLV_TIME_INTERVAL);
      reasonCode.TlvSerialize (buffer);
      // Fill in packet to send;
      packet = Create<Packet> (buffer.PeekData (), buffer.GetSize ());
      // Add Mih Header;
