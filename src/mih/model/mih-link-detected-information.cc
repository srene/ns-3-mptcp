...
namespace ns3 {
  namespace mih {
    ...
    LinkDetectedInformation::TlvSerialize (Buffer &buffer) const
    {
      ...
      TlvMih::SerializeU16 (buffer, m_sinr, TLV_SINR);
    ...
    LinkDetectedInformation::TlvDeserialize (Buffer &buffer) 
    {
      ...
      totalBytesRead += TlvMih::DeserializeU16 (buffer, m_sinr, TLV_SINR);
    ...
    {
      return TlvMih::GetSerializedSize<LinkDetectedInformation> (linkDetectedInformationList);
    }
    void
    TlvSerialize (Buffer &buffer, const LinkDetectedInformationList &linkDetectedInformationList)
    {
      TlvMih::Serialize<LinkDetectedInformation> (buffer, linkDetectedInformationList, TLV_LINK_DETECTED_INFORMATION_LIST);
    }
    uint32_t
    TlvDeserialize (Buffer &buffer, LinkDetectedInformationList &linkDetectedInformationList)
    {
      return TlvMih::Deserialize<LinkDetectedInformation> (buffer, linkDetectedInformationList, TLV_LINK_DETECTED_INFORMATION_LIST);      
    }    
    std::ostream&
