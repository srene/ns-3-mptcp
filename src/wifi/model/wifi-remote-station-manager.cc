...
uint16_t WifiRemoteStationManager::GetNConnectedSta (void) const
{	  NS_LOG_FUNCTION(this);
	  uint16_t count = 0;
	  for (StationStates::const_iterator i = m_states.begin (); i != m_states.end (); i++)
	    {
		  if((*i)->m_state == WifiRemoteStationState::GOT_ASSOC_TX_OK && (*i)->m_info.GetFrameErrorRate() < 0.6)
		  {
			  //NS_LOG_LOGIC("AP CONNECTED ADDRESSES " << (*i)->m_address);
			  //NS_LOG_LOGIC("M_INFO " << (*i)->m_info.GetFrameErrorRate());
			  count++;
		  }
