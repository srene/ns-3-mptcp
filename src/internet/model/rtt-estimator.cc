...
RttEstimator::RttEstimator ()
  : m_next (1), m_history (),
    ...
    m_multiplier (1)
{ 
  NS_LOG_FUNCTION (this << m_multiplier);
...
{ 
  NS_LOG_FUNCTION (this << m_next << seq << size << m_multiplier);
...
{ 
  NS_LOG_FUNCTION (this << ackSeq << m_multiplier);
  ...
  if (!h.retx && ackSeq >= (h.seq + SequenceNumber32 (h.count)))
    { // Ok to use this sample
	  ...
	  NS_LOG_LOGIC("Acked time " << Simulator::Now().GetSeconds() << " " << h.time.GetSeconds() << " " << m.GetSeconds());
...
{
  NS_LOG_FUNCTION (this << m_multiplier << m_maxMultiplier);
...
{
  NS_LOG_FUNCTION (this << m << m_multiplier);
  if (m_nSamples)
    { // Not first
