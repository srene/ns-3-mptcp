...
.AddAttribute ("Mtu", "Flow one time limit in ms",
			UintegerValue (1387),
			MakeUintegerAccessor (&MpKernelScheduler::m_mtu),
			MakeUintegerChecker<uint32_t> ())
;
  return tid;
}
...
MpKernelScheduler::GetPacket(std::vector<Ptr<TcpSocketBase> > sockets)
{
	...
	Ptr<Packet> packet;
	NS_LOG_FUNCTION(this << sockets[0]->AvailableWindow() << sockets[1]->AvailableWindow() << sockets[0]->GetMinRtt() << sockets[1]->GetMinRtt() << sockets.size() << m_lastUsedsFlowIdx);
	...
	if(map_it!=socket_list.end()){
		...
		for(std::map<uint32_t,Ptr<TcpSocketBase> >::iterator iter=  socket_list.begin(); iter!= socket_list.end();++iter)
		{
			...
			NS_LOG_LOGIC("insert " << iter->first << " " << iter->second <<  " " << socket->GetMinRtt() << " " << sock->GetMinRtt());
			if(socket->GetMinRtt()>sock->GetMinRtt()){
				socket=sock;
	...
	if((nextSubFlow!=-1)&&(m_txBuffer.TailSequence()>m_nextTxSequence)){
		...
		NS_LOG_LOGIC("Packet not sent " << packet);
...
void
MpKernelScheduler::SetNextSequence(SequenceNumber64 seq)
{
	m_nextTxSequence = seq;
}

int
//MpKernelScheduler::GetSubflowToUse(SequenceNumber64 seq, std::vector<Ptr<TcpSocketBase> > sockets,uint32_t lastUsedsFlowIdx)
MpKernelScheduler::GetSubflowToUse()
{
...
void
MpKernelScheduler::Update(std::vector<Ptr<TcpSocketBase> > sockets)
{
