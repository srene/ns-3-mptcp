...
{
  //std::cout << "length " << this << " " << (uint32_t)m_length << std::endl;
  return m_length;
}
...
uint8_t TcpHeader::GetOptionSize() const
{
...
uint32_t TcpHeader::Deserialize (Buffer::Iterator start)
{
  ...
  //std::cout << "Header length " << (uint32_t)m_length << " " << (uint32_t)(m_length - 5) * 4<< std::endl;
...
void TcpHeader::AppendOption (Ptr<TcpOption> option)
{
  ...
  {
	//std::cout << "SerializedSize " << (*i)->GetSerializedSize () << std::endl;
    totalLen += (*i)->GetSerializedSize ();
  };
	//std::cout << "totalen " << totalLen << std::endl;
  if (totalLen > 20) totalLen += 3;
	//std::cout << "totalen " << totalLen << std::endl;

  m_length = totalLen >> 2;
	//std::cout << "aDD APEND OPTION length " << this << " " << (uint32_t)m_length << " " << GetSerializedSize () << std::endl;
...
Ptr<TcpOption> TcpHeader::GetOption (uint8_t kind) const
{
  for (std::list<Ptr<TcpOption> >::const_iterator i = m_options.begin (); i != m_options.end () ; ++i)
    {
	  //std::cout << "option kind " << (*i)->GetKind () << std::endl;
      if ((*i)->GetKind () == kind)
        {
          return (*i);
        }
...
std::vector<Ptr<TcpOption> > TcpHeader::GetOptions (uint8_t kind) const
{
  std::vector<Ptr<TcpOption> > options;
  for (std::list<Ptr<TcpOption> >::const_iterator i = m_options.begin (); i != m_options.end () ; ++i)
    {
	  //std::cout << "option kind " << (*i)->GetKind () << std::endl;
      if ((*i)->GetKind () == kind)
