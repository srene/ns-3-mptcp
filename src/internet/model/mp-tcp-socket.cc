...
:
 ...
 // distribAlgo = Water_Filling;
  ...
  if(m_distribAlgo == Water_Filling){
	  m_scheduler = CreateObject<MpWaterfilling>();
  } else if(m_distribAlgo == Wardrop){
	  m_scheduler = CreateObject<MpWardrop>();
  } else if(m_distribAlgo == Round_Robin){
...
MpTcpSocket::ReceivedData(Ptr<TcpSocketBase> socket, SequenceNumber64 seq, Ptr<Packet> packet)
{
	...
	m_packetRxTrace(packet,fromAddress);
...
MpTcpSocket::Discard (SequenceNumber64 ack)
{
  NS_LOG_FUNCTION (this << ack );
	  ...
	  {
		 // NS_LOG_LOGIC("Seq erase " << it->first << " "<< (uint32_t)it->second);
		  seqTx.erase(it);
	  	}
  ...
  for(std::map<SequenceNumber64,uint8_t >::iterator iter = seqTx.begin(); iter != seqTx.end(); iter++)
  	{
	  NS_LOG_LOGIC("Seq acked " << iter->first << " "<< (uint32_t)iter->second);
  	}
  return 0;
}
...
MpTcpSocket::Send (Ptr<Packet> p, uint32_t flags)
{
  ...
  NS_LOG_FUNCTION (this << m_landaRate << p->GetSize());
  int i = -1;
  ...
  if(p->GetSize()>0){
	  ...
	  if(packet->GetSize()==0){
		  NS_LOG_LOGIC("Get packet Packet not sent");
		  return i;
	  }
	  if (m_state == ESTABLISHED || m_state == SYN_SENT || m_state == CLOSE_WAIT)
	  { // Try to send the data out
		  NS_LOG_LOGIC("Socket ESTABLISHED or CLOSE_WAIT");
		  //int lastUsedsFlowIdx = GetSubflowToUse(m_nextTxSequence);
		  ...
		  if(lastUsedsFlowIdx!=-1){
			  ...
			  NS_LOG_LOGIC("Drop packet");
			  m_drop(packet);
		  }

		  if(i>0){
			  if(m_distribAlgo==Water_Filling||m_distribAlgo==Wardrop){
...
DataDistribAlgo_t
MpTcpSocket::GetDistribAlgo (void) const
{
	return m_distribAlgo;
}

void
MpTcpSocket::SetDistribAlgo (DataDistribAlgo_t distribAlgo)
{
	m_distribAlgo = distribAlgo;
}
...
MpTcpSocket::NotifySubflowConnectionSucceeded(Ptr<TcpSocketBase> socket)
{
	...
	m_succeed = true;
	socket->SetSucceed();
	m_scheduler->SetMtu(m_sockets[0]->GetSegSize());
	if(m_distribAlgo==Water_Filling||m_distribAlgo==Wardrop)
