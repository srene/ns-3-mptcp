...
DSNMapping::DSNMapping (uint8_t sFlowIdx, uint64_t dSeqNum, uint16_t dLvlLen, SequenceNumber32 sflowSeqNum, SequenceNumber32 ack, Ptr<Packet> pkt)
{
...
DataBuffer::Add (uint8_t* buf, uint32_t size)
{
    ...
    uint32_t qty = 0;
    //NS_LOG_INFO("DataBuffer::Add");
    while( qty < toWrite )
    {
     //   NS_LOG_INFO("DataBuffer::Add2");

        buffer.push( buf[ qty ] );
        qty++;
    }
    //NS_LOG_INFO("DataBuffer::Add3");
...
Events_t MpTcpStateMachine::FlagsEvent (uint8_t f)
{
...
SA MpTcpStateMachine::Lookup (TcpStates_t s, Events_t e)
{
  NS_LOG_FUNCTION (this << s << e);
  //std::cout << this << printState(s) << " " << printEvent(e) << std::endl;
  return aT[s][e];
  //return SA (SYN_SENT, SYN_TX);
}
...
MpTcpSubFlow::MpTcpSubFlow()
    : routeId (0), state (CLOSED), phase (Slow_Start),
      ...
      oif (0), mapDSN (0), rtt(0), lastMeasuredRtt (Seconds (0.0))
{
    connected   = false;
...
MpTcpSubFlow::MpTcpSubFlow(SequenceNumber32 TxSeqNb)
    : routeId (0), state (CLOSED), phase (Slow_Start),
      sAddr (Ipv4Address::GetZero ()), sPort (0),
      dAddr (Ipv4Address::GetZero ()), dPort (0),
      oif (0), mapDSN (0), rtt(0),lastMeasuredRtt (Seconds (0.0))
...
void
MpTcpSubFlow::AddDSNMapping(uint8_t sFlowIdx, uint64_t dSeqNum, uint16_t dLvlLen, SequenceNumber32 sflowSeqNum, SequenceNumber32 ack, Ptr<Packet> pkt)
{
    NS_LOG_FUNCTION_NOARGS();
...
MpTcpAddressInfo::MpTcpAddressInfo()
    : addrID (0), netDev (0),ipv4Addr (Ipv4Address::GetZero ()), mask (Ipv4Mask::GetZero())
{
}
