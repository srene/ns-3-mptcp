...
{
  NS_LOG_FUNCTION (this<< m_rWnd.Get ()<< m_cWnd.Get ()<< m_ssThresh);
...
uint32_t
TcpWestwood::GetCongestionWindow (void)
{
  NS_LOG_FUNCTION (this);
  return m_cWnd.Get ();
...
{
  NS_LOG_FUNCTION (this << count << m_cWnd << m_minRtt.GetSeconds());
