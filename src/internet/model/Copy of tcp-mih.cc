...
TcpMih::TcpMih (void) : m_retxThresh (3), m_inFastRec (false), m_delay(0)
{
...
void
TcpMih::SetHandover (uint32_t delay, uint32_t n, uint32_t throughput, Mac48Address mac)
{
	...
	if(n<2)
		m_tcpMih->SetHandover(delay,n,4000000,mac);
	else
		m_tcpMih->SetHandover(delay,n,throughput,mac);
...
TcpMih::Handover (uint32_t delay, uint32_t n, uint32_t throughput)
{
	if(m_state == ESTABLISHED)
	{
		...
		m_delay = delay;
		uint32_t bdp = ((throughput/8) / n) * (m_validRtt.Get().GetSeconds()-(delay/1000000));

		NS_LOG_LOGIC (this << " delay " << delay << " throughput " << throughput << " n " << n << " m_validRtt " << m_validRtt.Get().GetMicroSeconds() << " cwnd " << m_cWnd << " bdp " << bdp);
