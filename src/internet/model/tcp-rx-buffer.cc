...
TcpRxBuffer::Add (Ptr<Packet> p, TcpHeader const& tcph)
{
  ...
  for (BufIterator i = m_data.begin (); i != m_data.end (); ++i)
    {
	  NS_LOG_LOGIC ("Buff seq="<<i->first << " m_nextRxSeq=" << m_nextRxSeq << " packet size=" << i->second->GetSize ());
  ...
  NS_LOG_LOGIC ("Updated buffer occupancy=" << m_size << " nextRxSeq=" << m_nextRxSeq << " availbytes=" << m_availBytes);
...
{
  NS_LOG_FUNCTION (this << maxSize << m_availBytes << m_data.size ());
  ...
  NS_LOG_LOGIC ("Extracted " << outPkt->GetSize ( ) << " bytes, bufsize=" << m_size
                             << ", num pkts in buffer=" << m_data.size () << " m_nextRxSeq=" << m_nextRxSeq);
  return outPkt;
}

Ptr<Packet>
TcpRxBuffer::Extract (SequenceNumber32 seq)
{
  NS_LOG_FUNCTION (this << m_availBytes << m_data.size ());

  NS_LOG_LOGIC ("Requested to extract " << seq << " bytes from TcpRxBuffer of size=" << m_size);
  if (seq.GetValue() == 0) return 0;  // No contiguous block to return
  NS_ASSERT (m_data.size ()); // At least we have something to extract
  Ptr<Packet> outPkt = Create<Packet> (); // The packet that contains all the data to return
  BufIterator i = m_data.begin ();
  uint32_t pktSize = i->second->GetSize ();
