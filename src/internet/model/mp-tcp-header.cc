...
void MpTcpHeader::Serialize (Buffer::Iterator start) const
{
  ...
  i.WriteHtonU32 (GetSequenceNumber().GetValue());
  i.WriteHtonU32 (GetAckNumber().GetValue());
...
NS_LOG_INFO("MpTcpHeader: optName == " << opt->optName);
      ...
      }else if(opt->optName == OPT_DSN)
      {
          ...
          i.WriteHtonU32(optDSN->subflowSeqNumber.GetValue());
...
NS_LOG_INFO("MpTcpHeader::Deserialize options found: " << kind << ", hlen = " << (int)hlen);
      ...
      {
          opt = new OptDataSeqMapping (kind, i.ReadU64(), i.ReadNtohU16(), SequenceNumber32(i.ReadNtohU32()));
          plen = (plen + 15) % 4;
          hlen -= 15;
      }else if(kind == OPT_TT)
      {
...
OptDataSeqMapping::OptDataSeqMapping(TcpOption_t oName, uint64_t dSeqNum, uint16_t dLevelLength, SequenceNumber32 sfSeqNum)
{
...
bool
MpTcpHeader::AddOptDSN(TcpOption_t optName, uint64_t dSeqNum, uint16_t dLevelLength, SequenceNumber32 sfSeqNum)
{
    NS_LOG_FUNCTION(this);
    if (optName == OPT_DSN)
    {
