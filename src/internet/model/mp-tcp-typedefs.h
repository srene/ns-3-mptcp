...
public:
    DSNMapping ();
    DSNMapping (uint8_t sFlowIdx, uint64_t dSeqNum, uint16_t dLvlLen, SequenceNumber32 sflowSeqNum, SequenceNumber32 ack, Ptr<Packet> pkt);
    //DSNMapping (const DSNMapping &res);
    virtual ~DSNMapping();
    uint64_t dataSeqNumber;
    uint16_t dataLevelLength;
    SequenceNumber32 subflowSeqNumber;
    SequenceNumber32 acknowledgement;
...
public:
    ...
    MpTcpSubFlow ();
    ~MpTcpSubFlow ();
    MpTcpSubFlow (SequenceNumber32 TxSeqNb);

    void StartTracing (string traced);
    void CwndTracer (double oldval, double newval);

    void AddDSNMapping(uint8_t sFlowIdx, uint64_t dSeqNum, uint16_t dLvlLen, SequenceNumber32 sflowSeqNum, SequenceNumber32 ack, Ptr<Packet> pkt);
    ...
    uint32_t ssthresh;
    SequenceNumber32 maxSeqNb;     // it represent the highest sequence number of a sent byte. In general it's egual to ( TxSeqNumber - 1 ) until a retransmission happen
    SequenceNumber32 highestAck;   // hightest received ACK for the subflow level sequence number
    uint64_t bandwidth;

    list<DSNMapping *> mapDSN;
    multiset<double> measuredRTT;
    //list<double> measuredRTT;
    Ptr<RttEstimator> rtt;
    Time     lastMeasuredRtt;
    SequenceNumber32 TxSeqNumber;
    SequenceNumber32 RxSeqNumber;
    ...
    SequenceNumber32 recover;
    uint8_t  ackCount;      // count of received acknowledgement after an RTO expiration
    SequenceNumber32 ReTxSeqNumber; // higher sequence number of the retransmitted segment
    int      nbRecvAck;
};

class MpTcpAddressInfo : public Object
{
public:
    MpTcpAddressInfo();
    ~MpTcpAddressInfo();
