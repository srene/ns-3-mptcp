 ...
 */
class TcpFreeze : public TcpSocketBase
{
public:
   ...
   */
  TcpFreeze (void);
  TcpFreeze (const TcpFreeze& sock);
  virtual ~TcpFreeze (void);

  // From TcpSocketBase
  virtual int Connect (const Address &address);
  virtual int Listen (void);
  virtual void handover (uint16_t delay);
  void Freeze();
  void Unfreeze();
protected:
  virtual uint32_t Window (void); // Return the max possible number of unacked bytes
  virtual Ptr<TcpSocketBase> Fork (void); // Call CopyObject<TcpFreeze> to clone me
...
protected:
  ...
  uint32_t               m_freezeSsThresh;     //< Slow Start Threshold
