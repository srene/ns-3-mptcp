 ...
 */
class TcpPartialMih : public TcpSocketBase
{
public:
   ...
   */
  TcpPartialMih (void);
  TcpPartialMih (const TcpPartialMih& sock);
  virtual ~TcpPartialMih (void);

  // From TcpSocketBase
  virtual int Connect (const Address &address);
  virtual int Listen (void);
  virtual void Handover (uint32_t delay, uint32_t n, uint32_t throughput);
  virtual void SetHandover (uint32_t delay, uint32_t n, uint32_t throughput, Mac48Address mac);
  void PostHandover ();
  void Freeze();
  void Unfreeze();
protected:
  ...
  virtual bool IsLost(SequenceNumber32 seq) const;
...
protected:
  ...
  uint32_t 				 m_delay;
  uint32_t				 m_freezeCwnd;
  uint32_t				 m_freezeSsThresh;
  uint32_t				 m_freezerWnd;
  bool					 m_freeze;
};
