...
Ptr<Socket> 
Socket::CreateSocket (Ptr<Node> node, TypeId tid, TypeId socketType, TypeId congestion)
{
  Ptr<Socket> s;
  NS_ASSERT (node != 0);
  Ptr<TcpSocketFactory> socketFactory = node->GetObject<TcpSocketFactory> (tid);
  NS_ASSERT (socketFactory != 0);
  s = socketFactory->CreateSocket (socketType,congestion);
  NS_ASSERT (s != 0);
  return s;
}
...
Socket::BindToNetDevice (Ptr<NetDevice> netdevice)
{
  ...
  NS_LOG_LOGIC ("BoundNetDevice " << m_boundnetdevice);
